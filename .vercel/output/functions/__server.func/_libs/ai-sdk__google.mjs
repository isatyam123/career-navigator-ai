import { $ as withoutTrailingSlash, A as isAbortError, B as mapReasoningToProviderBudget, D as getFromApi, E as generateId, Et as UnsupportedFunctionalityError, G as resolve, H as parseProviderOptions, I as isSameOrigin, J as safeParseJSON, K as resolveFullMediaType, M as isCustomReasoning, O as getRuntimeEnvironmentUserAgent, P as isFullMediaType, Q as withUserAgentSuffix, R as lazySchema, U as postJsonToApi, V as mapReasoningToProviderEffort, X as serializeModelOptions, b as createProviderExecutedToolFactory, bt as unknown, d as combineHeaders, dt as literal, et as zodSchema, f as convertBase64ToUint8Array, ft as looseObject, g as createEventSourceResponseHandler, gt as record, h as convertUint8ArrayToBase64, ht as object, k as getTopLevelMediaType, m as convertToBase64, mt as number, nt as WORKFLOW_SERIALIZE, ot as array, p as convertInlineFileDataToUint8Array, q as resolveProviderReference, rt as _enum, st as boolean, tt as WORKFLOW_DESERIALIZE, v as createJsonErrorResponseHandler, vt as string, wt as TooManyEmbeddingValuesForCallError, x as delay, xt as AISDKError, y as createJsonResponseHandler, yt as union, z as loadApiKey } from "./@ai-sdk/gateway+[...].mjs";
...
function appendLegacyToolResultParts(parts, toolName, outputValue, toolCallId) {
	for (const contentPart of outputValue) switch (contentPart.type) {
		case "text":
			...
			break;
		case "file":
			if (contentPart.data.type === "data" && getTopLevelMediaType(contentPart.mediaType) === "image") parts.push({ inlineData: {
				mimeType: resolveFullMediaType({ part: contentPart }),
				data: convertToBase64(contentPart.data.data)
...
function convertProviderReferenceImage(refImg) {
	...
	if (refImg.gcsUri) return { gcsUri: refImg.gcsUri };
	return refImg;
}
...
var GoogleRealtimeModel = class {
	...
	async doCreateClientSecret(options) {
		...
		const expireTime = new Date(now + openWindowMs + 1800 * 1e3).toISOString();
